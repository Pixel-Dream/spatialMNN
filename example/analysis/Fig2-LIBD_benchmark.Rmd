...
pdf(file = file.path("~/GitHub/atlasClustering/archived/fig",paste0(Sys.Date(),"_LIBD_edge_weight_stat.pdf")), width = 4, height = 3)
...
ggplot(data.frame(sample = sapply(names(seurat_ls),\(x){rep(x,nrow(seurat_ls[[x]]@misc[["edges"]]))}) %>% unlist(),
    ...
    geom_vline(aes(xintercept = 0.6), color = "red", linewidth = 1) + 
    annotate("text", label = "Cor. Threshold = 0.6", x = 0.28, y = 2.4, size = 5, colour = "black")+
