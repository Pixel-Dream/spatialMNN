...
pdf(file = file.path("~/GitHub/atlasClustering/archived/fig",paste0(Sys.Date(),"_STARmap_AD_edge_weight_stat.pdf")), width = 4, height = 3)

ggplot(data.frame(sample = sapply(names(seu_ls),\(x){rep(x,nrow(seu_ls[[x]]@misc[["edges"]]))}) %>% unlist(),
                  weight = sapply(seu_ls,\(obj){obj@misc[["edges"]]$weight}) %>% unlist()),
       aes(fill = sample, x = weight)) + 
    geom_density(alpha = 0.2) +
    geom_vline(aes(xintercept = 0.8), color = "red", linewidth = 1) + 
    annotate("text", label = "Cor. Threshold = 0.8", x = 0.4, y = 2.4, size = 5, colour = "black")+
...
for(i in levels(as.factor(rtn_ls[["seurat_obj"]]$sec_cluster_MNN))){
  plot_ls[[paste0("cl_",i)]] <- ggplot(all_markers %>% subset(cluster == i & pct.1 > 0.1 & pct.2 > 0.1) %>% 
       ...
       aes(x = avg_log2FC, y = -log10(p_val), color = de)) +
    geom_point() +
...
ggplot(all_markers %>% subset(cluster == 6 & pct.1 > 0.1 & pct.2 > 0.1) %>% 
                                         mutate(de = ifelse(p_val_adj < 0.01 & abs(avg_log2FC) > .1,ifelse(avg_log2FC > 0,"UP","DOWN"),"NO"),
                                                de_label = ifelse(gene %in% PIG_13mo$gene & de == "UP", gene, NA)),
       aes(x = avg_log2FC, y = -log10(p_val), color = de, label = de_label)) +
    geom_point() +
